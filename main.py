    ...
    try:
        r = requests.post(
            ...
            headers={
                ...
                "User-Agent": "Instagram 99.4.0 TweakPY_vv1ck",
                "Accept-Encoding": "gzip, deflate"
            },
            data={"signed_body": f"rand_sig.{{\"q\":\"{username}\"}}"}
        )
...
st.title("📢 Instagram Multi-Report Tool")
...
if submit:
    if not session_id or not csrf_token or not targets:
        ...
        st.success("🎉 Reporting complete!")import streamlit as st
import requests
import asyncio
import json
import time
...
def get_user_id(username, sessionid): try: r = requests.get( f"https://i.instagram.com/api/v1/users/web_profile_info/?username={username}", headers={ "User-Agent": "Instagram 155.0.0.37.107", "Cookie": f"sessionid={sessionid};", } ) return r.json()["data"]["user"]["id"] except: return None
